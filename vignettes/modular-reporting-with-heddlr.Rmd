...
However, maybe one day Joe down the hall points out that there are actually
...
some level of risk associated with that -- for instance, you can wind up 
reporting bad data if you miss replacing a value. More importantly, though, 
...
patterns. For instance, our example report is made up of two patterns. The 
first of those is the setup material:
...
The second is the species-specific section, which gets repeated for each
...
their own files. We'll refer to these patterns as `setup_pattern.Rmd` and
`species_pattern.Rmd` respectively^[In order for the vignettes for this package
to build correctly, I haven't actually saved these files off separately -- if 
you look at the source code for this vignette, you'll notice I'm not actually
using the `heddlr` functions, but rather using some workarounds to get the same
exact results. ].
...
which does more or less what you'd expect and import a single pattern into a 
single R object.
...
top-level environment. `heddlr` supports this out of the box via
`heddlr::assemble_draft()`, which will return a single list object holding all
of your patterns:

```{r eval=FALSE}
iris_draft <- assemble_draft("setup_pattern" = "setup_pattern.Rmd",
                             "species_pattern" = "species_pattern.Rmd")
```

```{r include=FALSE}
iris_draft <- list("setup_pattern" = setup_pattern,
...
in the same way as the other time we provided `heddle()` a dataframe:

```{r}
iris %>%
  nest(nested = Species) %>%
  mutate(component = map(nested, heddle, "This is a pattern - CODE ", "CODE" = Species)) %>%
  head(2)
```
...
`purrr:map()` might know that we can get a character output from most 
...
iris %>%
  nest(nested = Species) %>%
  mutate(component = map_chr(nested, heddle, "This is a pattern - CODE ", "CODE" = Species))
...
`stdout()` instead -- both in order to keep this vignette self-contained, and 
to show what our end report file will look like:
...
as our example here, but it can be a huge timesaver for more complex reports 
