test_that("heddle handles unexpected vector cases appropriately", {
  ...
  expect_error(heddle(iris$Species, "SPECIES CODE GWAR ", "GWAR", strip.whitespace = 1))
  expect_warning(heddle(iris$Species, "SPECIES CODE GWAR ", "GWAR " = "Species"))
})
...
test_that("heddle handles unexpected dataframe cases appropriately", {
  ...
  expect_error(heddle(spList4, "SPECIES CODE GWAR ", "GWAR" = Species, strip.whitespace = 1))
})

test_that("heddle works as expected on vectors", {
  spListChar <- as.vector(unique(iris$Species))
  spListFac <- unique(iris$Species)
  textOutput <- c("SPECIES CODE setosa ", "SPECIES CODE versicolor ", "SPECIES CODE virginica ")
...
test_that("heddle works as expected on data frames", {
  ...
  expect_match(heddle(spList4, "SPECIES CODE GWAR ", "GWAR" = Species)[[1]], "SPECIES CODE setosa")
  expect_match(heddle(spList4, "SPECIES CODE GWAR ", "GWAR" = Species, strip.whitespace = T)[[4]], "SPECIES CODE teststring")
  expect_match(heddle(spList4, "SPECIES CODE GWAR ", "GWAR" = Species, "CODE" = Species)[[1]], "SPECIES setosa setosa")
})
